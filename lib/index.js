...
/**
 * Returns a list of tokens found in the provided string.
 * @param {string} string
 * @return {Array<string>}
 */
...
/**
 * Converts the provided word into its metaphone description.
 * @param {string} string
 * @return {string}
 */
function metaphone(string) {
    ...
    return string[0] + string.substr(1).replace(/[aeiou]/g, "");
}
...
function index(file, filename, options, searchData) {
    const entry = {},
        entryIndex = searchData.entries.length;

    if (file.title) {
        if (Buffer.isBuffer(file.title)) {
            entry.title = file.title.toString("utf8");
        } else {
    ...
    entry.date = file.date && file.date.toLocaleString("en-US", {month: "long",
        year: "numeric"});
    ...
    Object.keys(options.index).forEach((metadataName) => {
        ...
        for (const term of tokens(value)) {
            ...
            mp = metaphone(term);
            while (mp.length > 1) {
                if (Object.prototype.hasOwnProperty.call(searchData.index, mp)) {
